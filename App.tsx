...
const App: React.FC = () => {
  ...
  const generateQuestion = useCallback((selectedLevel: GameLevel) => {
    // 1. Filtrar personajes para no repetir el anterior inmediatamente
    const availableCharacters = CHARACTERS.filter(c => c.id !== lastCorrectId.current);
    
    // 2. Seleccionar el personaje CORRECTO (objetivo)
    ...
    // 3. Seleccionar otros 3 personajes INCORRECTOS aleatorios
    const otherOptions = CHARACTERS
      .filter(c => c.id !== correctAnswer.id)
      .sort(() => 0.5 - Math.random())
      .slice(0, 3);

    // 4. Mezclar las opciones (1 correcto + 3 incorrectos)
    const options = [correctAnswer, ...otherOptions].sort(() => 0.5 - Math.random());

    // 5. Construir la descripción BASADA ÚNICAMENTE EN LOS RASGOS DEL CORRECTO
    ...
    // Definir cuántos rasgos mostrar según el nivel
    let numTraitsToShow = 3;
    if (selectedLevel === GameLevel.MEDIUM) {
      numTraitsToShow = 4; // Mostrar casi todos para mayor detalle
    }

    // Elegir rasgos aleatorios del personaje correcto para la descripción
    const selectedTraits = [...allPossibleTraits].sort(() => 0.5 - Math.random()).slice(0, numTraitsToShow);

    let description = "";
    if (selectedTraits.length > 0) {
      const lastTrait = selectedTraits.pop();
      const initialTraits = selectedTraits.join(", ");
        ...
        <div className="w-full flex justify-between items-center mb-10 bg-white/50 backdrop-blur-sm p-4 rounded-3xl border border-white shadow-sm">
          ...
          <div className="flex gap-4">
            <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border-b-4 border-green-500">
              <span className="text-xs uppercase text-slate-400 block font-bold mb-1">Puntos</span>
            ...
            <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border-b-4 border-blue-500">
              <span className="text-xs uppercase text-slate-400 block font-bold mb-1">Ronda</span>
              <span className="font-bold text-2xl text-blue-600">{totalRounds + 1}/10</span>
            </div>
          </div>
        </div>

        {/* Panel de Pregunta */}
        <div className="w-full max-w-4xl bg-white px-10 py-12 rounded-[4rem] shadow-2xl border border-blue-50 mb-12 text-center relative overflow-hidden flex flex-col items-center justify-center">
  ...
  const renderSummary = () => (
    <div className="flex flex-col items-center justify-center min-h-screen text-center p-6 bg-slate-50">
      ...
      <div className="bg-white p-16 rounded-[4rem] shadow-2xl border-t-[12px] border-blue-500 max-w-lg w-full transform transition-all hover:scale-[1.02]">
        ...
        <h2 className="title-font text-5xl text-blue-600 mb-4">¡Victoria!</h2>
