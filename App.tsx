...
const App: React.FC = () => {
  ...
  const renderStart = () => (
    <div className="flex flex-col items-center justify-center min-h-[85vh] text-center p-6">
      <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl border-b-8 border-blue-200 w-full max-w-xl">
  ...
  const renderPlaying = () => {
    ...
    return (
      <div className="flex flex-col items-center min-h-screen p-4 max-w-5xl mx-auto">
        ...
        {/* Panel de Descripción con Padding corregido */}
        <div className="w-full bg-white p-10 pt-16 rounded-[2.5rem] shadow-xl border-4 border-blue-50 mb-8 text-center relative overflow-hidden">
          <div className="bg-blue-500 text-white px-5 py-1.5 rounded-full text-xs font-bold absolute top-4 left-1/2 -translate-x-1/2 uppercase tracking-widest whitespace-nowrap shadow-md">
            Busca a la persona
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
          {currentQuestion.options.map((char) => (
            <button
              ...
              className={`group relative aspect-square bg-white rounded-[1.5rem] border-4 transition-all overflow-hidden shadow-md ${
            ...
            >
              <img
                src={char.image}
                alt={`Personaje ${char.id}`}
                className="w-full h-full object-cover transition-transform group-hover:scale-110"
                loading="eager"
                onError={(e) => {
                   // Fallback visual con mensaje de depuración si la imagen falla
                   console.error("Error cargando imagen:", char.image);
                   (e.target as HTMLImageElement).src = `https://placehold.co/400x400/f1f5f9/64748b?text=Img+${char.id}`;
                }}
              />
            </button>
          ))}
